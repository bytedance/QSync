...
from qsync.LpTorch import QModule, QProfiler, QProfiler_Trans
...
model_name = 'bert'
profile_folder = os.path.join(os.environ['HOME'], "profile_result")
...
def parse_args():
    ...
    parser.add_argument('--tbitwidth', type=int, default=None, help='test_case')
    parser.add_argument('--eval_iteration', type=int, default=200, help='test eval')
    parser.add_argument('--indicator_type', type=int, default=0, help='ind type')
    parser.add_argument('--dy-bs', type=int, default=0, help='ind type')
...
def main():
    ...
    model = QModule(model, indicator_type=args.indicator_type)
    model.load_and_set_depth(model_name)
    print("arg path", args.mapper_path)
    

    arch_num = str(get_capability())
    profile_path = os.path.abspath(os.path.join(profile_folder, f"profile_data_{model_name}_{arch_num}_{qconfig.indicator_type}.npz"))
    if args.profile_only:
        ...
        QProfiler_Trans(model, train_dataloader, rank=0, profile_iter=10, \
                available_bits=model.available_bits, profiled_data_path=profile_path, optimizer=optimizer)
        exit()
    ...
    if args.test_case is not None:
        ...
        if test_case_num == 1:
            ...
            target_layer = model.model.bert.encoder.layer[0]
            model.set_bits_in_layers(16, target_layer)
            target_layer = model.model.bert.encoder.layer[2]
            model.set_bits_in_layers(16, target_layer)
            target_layer = model.model.bert.encoder.layer[4]
            model.set_bits_in_layers(16, target_layer)
            
            exp_name = 'fp16_layer3_comm_t4'

        elif test_case_num == 5:
            for qli in model.qli_list:
                qli.reset_bits(16)
            exp_name = 'fp16_li_comm_t4'
        
        elif test_case_num == 6:
            for qli in model.qli_list:
                qli.reset_bits(8)
            exp_name = 'int8_li_comm_t4'
        ...
        # same with case 2 but use DDP
        elif test_case_num == 8:
            model.set_bits(16)
            exp_name = 'fp16_ddp_t4'
    
    if args.tbitwidth is not None:
        if args.tbitwidth == 16:
            for qemb in model.qemb_list:
                qemb.reset_bits(16)
            for qli in model.qli_list:
                qli.reset_bits(16)
            for qmatmul in model.qmatmul_list:
                qmatmul.reset_bits(16)
        else:
            for qemb in model.qemb_list:
                qemb.reset_bits(16)
            # print(model.qli_list)
            for qli in model.qli_list:
                qli.reset_bits(8)
            for qmatmul in model.qmatmul_list:
                qmatmul.reset_bits(16)
            for qln in model.qln_list:
                qln.reset_bits(16)
        # for qmatmul in model.qmatmul_list:
        #     qmatmul_list.reset_bits(16)

    if args.dy_bs is None:
        args.dy_bs = args.per_device_train_batch_size
    ...
    def eval_model():
        ...
        progress_bar_eval = tqdm(range(len(eval_dataloader)), disable=not accelerator.is_local_main_process)
        ...
        # del all_end_logits
        # all_start_logits.empty()
        # all_end_logits.empty()
    ...
    def change_bs_batch(batch):
        if args.dy_bs is None: return dict(batch), 6/5
        # ratio = args.dy_bs / (( args.per_device_train_batch_size + args.dy_bs) / 2)
        ratio = 1
        dict_batch = dict(batch)
        input_ids = dict_batch['input_ids']
        token_type_ids = dict_batch['token_type_ids']
        attention_mask = dict_batch['attention_mask']
        start_pos = dict_batch['start_positions']
        end_pos = dict_batch['end_positions']

        input_ids =input_ids[:args.dy_bs]
        token_type_ids = token_type_ids[:args.dy_bs]
        attention_mask = attention_mask[:args.dy_bs]
        start_pos = start_pos[:args.dy_bs]
        end_pos = end_pos[:args.dy_bs]
        dict_batch['input_ids'] = input_ids
        dict_batch['token_type_ids'] = token_type_ids
        dict_batch['attention_mask'] = attention_mask
        dict_batch['start_positions'] = start_pos
        dict_batch['end_positions'] = end_pos
        
        return dict_batch, ratio
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.resume_from_checkpoint and epoch == starting_epoch:
                if resume_step is not None and step < resume_step:
                    completed_steps += 1
                    continue
            # model.advance_q_weight_calculate()
            ratio = 1.0
            batch, ratio = change_bs_batch(batch)
            ...
            if step % args.eval_iteration == 1 and step !=1:
                eval_model()
