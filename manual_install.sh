...
cd $PYTHONPATH
apt-get update && apt-get install -y nano wget tmux numactl libnuma-dev build-essential libc6-dev iputils-ping libcudnn8 libcudnn8-dev libnccl2 libnccl-dev
...
pip3 install transformers -i https://pypi.doubanio.com/simple
echo "end"
make